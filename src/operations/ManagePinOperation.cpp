...
void ManagePinOperation::doWork()
{
    ...
    // Emit the typed credential result for EVERY completed attempt
    // (invalidPin/blocked carry retries_left/blocked) BEFORE Finished. An
    // undeliverable Result fails the op CLOSED (contract: ordering holds modulo
    // channel allocation failure) — never Finished(Ok) with a silently-missing
    // payload.
