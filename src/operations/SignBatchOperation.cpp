...
void SignBatchOperation::doWork()
{
    ...
    if (result.outcome == BatchSignFlow::Outcome::InvalidRequest) {
        // Defense-in-depth only: the host validates the document
        // count at its batch method entry (mirroring BatchSignFlow's own
    ...
    // BEFORE Finished, mirroring every sibling typed result. Unlike a single
    // sign, this fires even when the AGGREGATE status is Error: a batch conveys
