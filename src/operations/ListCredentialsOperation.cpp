...
// typed credential result carries. Finished keeps the flow's richer
...
void ListCredentialsOperation::doWork()
{
    ...
    // Emit the typed credential result for EVERY completed list attempt BEFORE
    // Finished — the interface promises the payload "for every completed
    // attempt", mirroring the mutations: outcome=ok + the records on success,
    // the mapped outcome token + empty records for a cancel or failure (a client
