...
std::optional<CardReadSnapshot> GetPhotoOperation::obtainSnapshot()
{
    ...
    // object here yields a photo result, not an identity one -- there is no
    // group stream to emit, so progressive delivery is gated OFF at this wiring
    // site (see NullGroupSink's own doc comment), never inside the flow or the
    // channel.
...
void GetPhotoOperation::doWork()
{
    ...
    // Collect the raw photo bytes keyed by "groupKey:fieldKey". The host
    // channel's sealed handoff carries each one out; the core stays
    // transport-neutral and yields plain bytes.
    PhotoResult photos;
    ...
    // Emit the typed photo result BEFORE Finished (strict ordering contract). A
    // sealed-handoff failure returns false: fail the op CLOSED rather than emit
    // Finished(Ok) with no result.
