...
// wire-ordering contract: the typed Result is
// available for EVERY completed attempt — success, soft-fail, user cancel and
// hard failure alike — always BEFORE Finished; and a Result the
...
// Fields are ASSIGNED, not designated-initialised: PINResult declares
// retriesLeft BEFORE outcome, and a designated initialiser in the reading order
// is a hard compile error.
LibreSCRS::Plugin::PINResult makePinResult(LibreSCRS::Plugin::PINResultOutcome outcome,
...
// unspecified, empty records) BEFORE Finished(Error): the interface promises the
...
// --- ported: the two cases only one host's copy ever had ---------------------
//
// Both existed in a single backend's duplicate of this suite and in neither
// other copy, which is exactly the pair a move by file name loses. Their names
// are unchanged, because comparing sets of test identities only works when no
// name is translated.

// A cancelled change prompt still emits exactly one Result (outcome
...
TEST(ManagePinOperation, CancelledPromptEmitsUserCancelledResultThenFinishesCancelled)
{
    ...
    expectSingleResultThenFinish(*raw);
    EXPECT_EQ(raw->results[0].op.outcome, CredentialOutcome::UserCancelled);
...
{
    Harness h;
    h.credentials.changePinResult = makePinResult(LibreSCRS::Plugin::PINResultOutcome::InvalidPin, 2);
