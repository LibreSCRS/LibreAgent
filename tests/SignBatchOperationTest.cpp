...
//
// Hermetic exercise of SignBatchOperation (the batch dispatch's worker-side
...
// Finished, and that a FRESH BatchPinHolder backs every
...
// rows into. Sealing them (incl. the zero-length convention for a
// failed row) is the BACKEND channel's job (the host's operation factory
...
TEST(SignBatchOperation, AllRowsFailIndependentlyFinishesError)
{
    ...
    // The rows are STILL delivered (unlike a single sign, which never fires on
