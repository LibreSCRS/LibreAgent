...
//
// Hermetic exercise of SignFlow. Every seam is a Fake; the flow runs
// synchronously on the test thread. This covers the orchestration: the unified
// PIN/CAN credential lambda, the watchdog-safe phase ordering, status mapping
// and cache eviction.
//
// The result-recovery store the host publishes for a late subscriber is not
// here: it seals a descriptor, which only one platform can do, so those cases
// live with that platform's store.
//
...
struct Harness
{
    ...
    // The resolved level on the request handed to the flow. The host's sign entry
...
// Records whether the op emitted a sign result and its terminal finish, so the
...
TEST(SignFlow, TsaUnreachableDoesNotEmitResult)
{
    ...
    // a TsaUnreachable outcome must finish the op Error and NEVER emit a sign
