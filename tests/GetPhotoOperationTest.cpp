...
public:
    ...
    {
        // The core yields raw photo bytes (the op seals nothing); the backend
        // channel is what seals them. Capture the raw bytes keyed by
        // "groupKey:fieldKey". `deliver` false models a backend sealing failure
...
TEST(GetPhotoOperation, CacheHitSkipsFlowAndEmitsPhoto)
{
    ...
    // The op yields the raw JPEG bytes verbatim; sealing them is the
    // backend channel's job, not the op's.
    EXPECT_EQ(raw->captured.at("photo:portrait"), (std::vector<std::uint8_t>{0xFF, 0xD8, 0xFF, 0xE0}));
}

// Restored fail-closed contract: when the backend channel's sealing fails
// (emitResult returns false), the op finishes Error(CommunicationError,
// "op.memfd_failed") rather than leaking Finished(Ok) with no photo result.
