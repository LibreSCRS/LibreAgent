...
// Agent-owned wire enum for the certificate chain/trust verdict (spec §3.2).
...
// ChainStatus addition can never silently alias it. Until Increment 6 wires the
// chain ladder + TrustStore verdict, ReadCertificates emits Unknown (the chain
// is "not evaluated").
...
// crosses the wire (spec D1): the agent does ALL X.509 parsing and ships
...
struct CertSnapshot
{
    ...
    std::vector<std::string> chainSubjectCns{}; // leaf..root subject CNs (display); Inc 2: [leaf CN]
