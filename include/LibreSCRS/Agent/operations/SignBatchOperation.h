...
public:
    struct Deps
    {
        ...
        // resolved ONCE at the host's batch method entry exactly like a
...
private:
    ...
    // — one per batch dispatch, built by the host's operation factory — so it
    // is NEVER shared or reused across batches; BatchSignFlow's own scope
    // guard wipes it on every run() exit
