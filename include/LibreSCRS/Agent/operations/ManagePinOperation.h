...
// only exists for a resolvable request). Emits the typed credential result
// (outcome + retries_left / blocked / ...) for every completed attempt, then
// Finished with the mapped status/errorCode.
...
public:
    struct Deps
    {
        ...
        std::string artifact;
        // Resolved at the host's PIN-management entry; the flow consumes both.
